...
class SettingsRegistry {
	...
	function register_field( string $section, array $field ) {
		 ...
		 */
		$field = apply_filters( 'c_purge_cache_setting_field_config', $field_config, $field_name, $section );
	...
	function admin_init() {
		...
		do_action( 'c_purge_cache_init_settings', $this );
		 ...
		 */
		$setting_sections = apply_filters( 'c_purge_cache_settings_sections', $this->settings_sections );
			...
			<?php foreach ( $this->settings_sections as $id => $form ) { ?>
				<div id="<?php echo esc_attr( $id ); ?>" class="group" style="display: none;">
					<form method="post" action="options.php">
						<?php
						do_action( 'c_purge_cache_settings_form_top', $form );
						settings_fields( $id );
						do_settings_sections( $id );
						do_action( 'c_purge_cache_settings_form_bottom', $form );
